...
This repo originally belongs to [Shishir Patel](https://github.com/ShishirPatil)'s [Gorrilla RAFT](https://github.com/ShishirPatil/gorilla/tree/main/raft).
...
Right now just pdf docs are supported. 

Command Parameters used by me :

```commandline
python3 raft.py     --datapath $PWD/sample_data/UC_Berkeley.pdf     --output $PWD/output     --distractors 3     --doctype pdf     --chunk_size 512     --questions 5   --completion_model meta.llama3-70b-instruct-v1:0 --hf_embedding_model BAAI/bge-small-en-v1.5 --aws_region_name us-east-1 --aws_access_key ASJDASKDALSANFASF --aws_secret_access_key KASKDASFfasndasd/adsdadASD
...
- `--aws_region_name` - AWS Region in which Bedrock is defined.
- `--aws_access_key `- AWS Access key id assigned to you or your IAM role.
- `--aws_secret_access_key` - AWS Secret Access key assigned to you or your IAM role.
- `--openai_embedding_model` - The embedding model to use to encode documents chunks (text-embedding-ada-002, ...)
- `--hf_embedding_model-huggingface` -  model id to be used in the embedding model
