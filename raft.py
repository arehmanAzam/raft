...
from bedrock_call import bed_generate_instructions_gen, bedrock_inference,get_bedrock_client
log_setup()
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--aws_region_name", type=str, default=None, help="AWS Region in which Bedrock is defined")
    parser.add_argument("--aws_access_key", type=str, default=None, help="AWS Access key id assigned to you or your IAM role")
    parser.add_argument("--aws_secret_access_key", type=str, default=None, help="AWS Secret Access key assigned to you or your IAM role")
    parser.add_argument("--openai_embedding_model", type=str, default="text-embedding-ada-002", help="The embedding model to use to encode documents chunks (text-embedding-ada-002, ...)")
    parser.add_argument("--hf_embedding_model", type=str, default="text-embedding-ada-002", help="huggingface model id to be used in the embedding model")
    parser.add_argument("--completion_model", type=str, default="gpt-4", help="The model to use to generate questions and answers (meta.llama3-70b-instruct-v1:0,gpt-3.5, gpt-4, ...)")
...
def get_chunks(
    ...
    if doctype == "api":
        ...
        if openai_key !=None:
            ...
            embeddings= build_langchain_embeddings(model_name=model,cache_folder='./models')
...
def generate_label(client: OpenAI, question: str, context: Any, doctype: DocType = "pdf", model: str = None) -> str | None:
    ...
    return response
def bed_generate_label(client,question: str, context: Any) -> str | None:
    """
    Generates the label / answer to `question` using `context` and bedrock model.
    """
    prompts = []
    ...
    response=bedrock_inference(client,prompt)
...
def add_chunk_to_dataset(
    ...
    if doctype=="api":
        qs=generate_instructions(client, chunk, x, model)
    elif doctype!="api" and type(client)==OpenAI:
        qs=generate_instructions_gen(client,chunk,x,model)
    else:
        qs=bed_generate_instructions_gen(client,chunk, x, model)
    if qs != None:
        for q in qs:
            datapt = {
                "id": None,
            ...
            if type(client)==OpenAI:
                ...
                datapt["cot_answer"] = bed_generate_label( client,q,chunk)
...
def main():
    ...
    client=None
    if 'gpt' in args.completion_model:
        OPENAPI_API_KEY = args.openai_key

        client = build_openai_client(
            api_key=OPENAPI_API_KEY,
        )
        chunks = get_chunks(args.datapath, args.doctype, CHUNK_SIZE, OPENAPI_API_KEY, model=args.openai_embedding_model)
    else:
        client = get_bedrock_client(args.aws_region_name, args.aws_access_key, args.aws_secret_access_key)
        chunks = get_chunks(args.datapath, args.doctype, CHUNK_SIZE, model=args.hf_embedding_model)
